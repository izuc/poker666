...
class Table extends TableSubject
{
	...
	public function compareTwoPlayers(Player $first, Player $second) {
        $evaluator = new HandEvaluator();
		$communityCards = $this->getCommunityCards();
		
		$first_cards = new CardCollection();
		$first_hand = $first->getHand();
		$first_cards->mergeCards($first_hand);
		
		if (!empty($communityCards)) {
			$first_cards->mergeCards($communityCards);
		}
		
		$first_hand->setStrength($evaluator->getStrength($first_cards));
				
		$second_cards = new CardCollection();
		$second_hand = $second->getHand();
		$second_cards->mergeCards($second_hand);
		
		if (!empty($communityCards)) {
			$second_cards->mergeCards($communityCards);
		}
		
		$second_hand->setStrength($evaluator->getStrength($second_cards));
		
		
		return $evaluator->compareTwoHands($first_hand->getStrength(), $second_hand->getStrength());
    }
	
	public function getWinners() {
		
		if (!empty($this->players)) {
			usort($this->players, [$this, 'compareTwoPlayers']);
			return $this->players;
		}
